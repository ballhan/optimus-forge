...
// Stage, lighting, post-processing and controls shared by every study.
// Each entry point supplies its own model, framing distances and captions.
export function createViewer({ build, framing, copy }) {
  ...
  function reset() {
    ...
    const framingDrop =
      framing.vehicleDrop * THREE.MathUtils.smoothstep(progress, 0.4, 1);
    const [x, y, z] = mobile ? framing.mobile : framing.desktop;
    camera.position.set(x, y - framingDrop, z);
    controls.target.set(0, framing.robotTarget - framingDrop, 0);
    controls.update();
  }
  document.querySelector("#detail").addEventListener("click", () => {
    ...
    const view = progress > 0.7 ? framing.inspectVehicle : framing.inspectRobot;
  ...
  function animate(now) {
    ...
    progress = reduced
      ? target
      : THREE.MathUtils.damp(progress, target, 13, dt);
    ...
    const oldArmor = armor,
      oldGrip = grip;
    armor = THREE.MathUtils.damp(
      armor,
      Number(armorControl.value) / 100,
      10,
      dt,
    );
